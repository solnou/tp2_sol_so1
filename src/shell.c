...
int init_shell(int argc, char* argv[])
{
    ...
    if (argc > 1)
    { // se envio un bathc file
        // Leer comandos desde el archivo
        FILE* file = fopen(argv[1], "r"); // r indica que se va a leer (Read) el archivo
        if (file == NULL)
        {
        ...
        char command[256];
        // el while ejecuta todos los comandos del archivo, fgets se encarga de saltar de linea en cada bucle
...
void choose_execution(char* command)
{
    ...
    else if (strchr(command, '|') != NULL) // verifica si hay alguna | en el codigo
    {

        execute_piped_commands(command);
    }
    else if (strchr(command, '<') != NULL || strchr(command, '>') != NULL) // verifica si hay algun <> en el codigo
    {

        execute_command_redirection(command);
    }
    else
    { // si no hay ningun simbolo especial, es un comando normal
