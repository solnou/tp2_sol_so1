...
void start_monitor()
{
    if (monitor_pid == -1) // Verifica si el monitor no está en ejecución
    {
        ...
        if (monitor_pid == 0) // Proceso hijo
        {
            ...
            int devnull = open("/dev/null", O_RDWR); // Abre /dev/null para redirigir la salida
        ...
        else // Proceso padre
        {
            ...
            printf("Monitor iniciado.\n"); // Imprime un mensaje indicando que el monitor ha iniciado
        }
    }
    else // Si el monitor ya está en uso
    {
        printf("El monitor ya está en uso.\n"); // Imprime un mensaje indicando que el monitor ya está en uso
    }
}
...
void stop_monitor()
{
    ...
    {
        kill(monitor_pid, SIGTERM);    // Envía una señal SIGTERM al proceso del monitor para detenerlo
        waitpid(monitor_pid, NULL, 0); // Espera a que el proceso del monitor termine
        monitor_pid = -1;              // Resetea el PID del monitor a -1 indicando que no está en ejecución
        monitoring = 0;                // Marca que el monitor no está en ejecución
        printf("Monitor detenido.\n"); // Imprime un mensaje indicando que el monitor ha sido detenido
    }
    else // Si el monitor no está en ejecución
    {
        printf(
            "El monitor no está en ejecución.\n"); // Imprime un mensaje indicando que el monitor no está en ejecución
    }
}

void status_monitor()
{
    const char* fifo_path = "/tmp/monitor_fifo"; // Ruta del FIFO para comunicación entre procesos
    ...
    ssize_t bytes_read = read(fifo_fd, buffer, sizeof(buffer) - 1); // Lee datos del FIFO
    if (bytes_read > 0)                                             // Si se leyeron datos
    {
        ...
        if (json)                          // Si el JSON fue parseado correctamente
        {
            ...
            cJSON* cpu_usage = cJSON_GetObjectItem(json, "cpu_usage");         // Obtiene el uso de CPU
            cJSON* memory_usage = cJSON_GetObjectItem(json, "memory_usage");   // Obtiene el uso de memoria
            cJSON* disk_usage = cJSON_GetObjectItem(json, "disk_usage");       // Obtiene el uso de disco
            cJSON* network_usage = cJSON_GetObjectItem(json, "network_usage"); // Obtiene el uso de red
            cJSON* process_count = cJSON_GetObjectItem(json, "process_count"); // Obtiene el número de procesos
            cJSON* context_switches =
                cJSON_GetObjectItem(json, "context_switches"); // Obtiene las interrupciones de contexto

            // Mostrar valores según la opción seleccionada
            if (option == 1 && cpu_usage) // Si la opción es 1 y se obtuvo el uso de CPU
            {
                printf("Uso de CPU: %.2f%%\n\n", cpu_usage->valuedouble); // Imprime el uso de CPU
            }
            if (option == 2 && memory_usage) // Si la opción es 2 y se obtuvo el uso de memoria
            {
                printf("Uso de Memoria: %.2f MB\n\n", memory_usage->valuedouble); // Imprime el uso de memoria
            }
            if (option == 3 && disk_usage) // Si la opción es 3 y se obtuvo el uso de disco
            {
                printf("Uso de Disco: %.2f GB\n\n", disk_usage->valuedouble); // Imprime el uso de disco
            }
            if (option == 4 && network_usage) // Si la opción es 4 y se obtuvo el uso de red
            {
                printf("Uso de Red: %.2f KB/s\n\n", network_usage->valuedouble); // Imprime el uso de red
            }
            if (option == 5 && process_count) // Si la opción es 5 y se obtuvo el número de procesos
            {
                printf("Número de Procesos: %d\n\n", process_count->valueint); // Imprime el número de procesos
            }
            if (option == 6 && context_switches) // Si la opción es 6 y se obtuvieron las interrupciones de contexto
            {
                printf("Cambios de Contexto: %d\n\n",
                       context_switches->valueint); // Imprime las interrupciones de contexto
            }
