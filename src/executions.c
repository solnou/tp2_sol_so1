...
void execute_command(char *command) {
  ...
  // En el caso de que no se llame a ninguno de los comandos listados en el
  // arreglo de internals_commands
...
void execute_piped_commands(char *command) {
  ...
  while (command_n != NULL) { // mientras haya | para separar, va dividiendo
    commands[num_commands++] = command_n;
    command_n = strtok(NULL, "|");
  }

  // 2. Crear el pipe para cada par de comandos
  int *filedes = malloc(2 * (num_commands - 1) *
                        sizeof(int)); // Usar memoria dinámica
                                      // lea y ottro que escriba hacemos *2
  for (int j = 0; j < num_commands - 1; j++) {
    if (pipe(filedes + j * 2) < 0) { // Crea los pipes y veriifica que no fallen
      perror("pipe");
      exit(EXIT_FAILURE);
    }
  }
  ...
  for (int i = 0; i < num_commands; i++) {
    ...
    if (pid == 0) {
      ...
      if (j != 0) {
        // Saltenado el primer comando, redirigimos la entrada estandar al pipe
      ...
      perror("execvp");
      exit(EXIT_FAILURE);
    } else if (pid < 0) { // problemas al crear el proceso hijo
      perror("fork");
      exit(EXIT_FAILURE);
    }
    j += 2;
...
void execute_command_redirection(char *command) {
  ...
  while (token != NULL) {
    if (strcmp(token, "<") ==
        0) { // si encontramos < el siguiente token es el archivo de entrada
      token = strtok(NULL, " ");
      if (token != NULL) {
        input_file = token;
      }
    } else if (strcmp(token, ">") == 0) { // si encontramos > el siguiente token
      ...
      if (program == NULL) { // el primer token, sin haber encontrado ningun <>,
                             // es el programa
        program = token;
      }
